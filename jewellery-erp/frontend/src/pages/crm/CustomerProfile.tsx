...
export default function CustomerProfile({ id, onBack }: { id: number, onBack: () => void }) {
  ...
  const fetchData = async () => {
    try {
      const [custRes, ledgerRes] = await Promise.all([
        ...
        axiosClient.get(`/customers/${id}/ledger`)
      ]);
  ...
  const handleAddEntry = async () => {
    try {
      await axiosClient.post(`/customers/${id}/ledger`, form);
  ...
  if (loading || !customer) return <div className="p-8 text-primary font-bold">Loading...</div>;
    ...
    <div className="h-[calc(100vh-6rem)] flex flex-col space-y-4">
      ...
      <div className="bg-surface border border-gray-800 rounded-xl p-6 shadow-lg flex items-start justify-between">
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="text-right">
            <div className="text-[10px] font-bold text-textMuted uppercase tracking-wider">Outstanding Balance</div>
            <div className={`text-3xl font-bold font-mono ${customer.outstanding_balance > 0 ? 'text-red-400' : 'text-green-400'}`}>
              ₹ {Math.abs(customer.outstanding_balance).toLocaleString()} {customer.outstanding_balance > 0 ? '(Dr)' : '(Cr)'}
            </div>
          </div>
          <div className="text-right">
            <div className="text-[10px] font-bold text-textMuted uppercase tracking-wider">Credit Limit</div>
            <div className="text-sm font-bold font-mono text-primary">₹ {customer.credit_limit.toLocaleString()}</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 bg-surface border border-gray-800 rounded-xl shadow-lg flex flex-col overflow-hidden">
        <div className="p-4 border-b border-gray-800 flex justify-between items-center">
          <h2 className="text-lg font-bold text-primary uppercase tracking-widest">Account Ledger</h2>
        ...
        {showEntryForm && (
          <div className="p-4 bg-black/40 border-b border-gray-800 grid grid-cols-5 gap-4 items-end">
            ...
            <div className="col-span-2">
              ...
              <input value={form.description} onChange={e=>setForm({...form, description: e.target.value})} className="w-full bg-background border border-gray-700 rounded p-2 text-sm text-textMain focus:border-primary outline-none" placeholder="e.g. Cash received" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Debit (Bill) ₹</label>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Credit (Pay) ₹</label>
            ...
            <tbody className="divide-y divide-gray-800/50">
              {ledger.map((row) => (
                <tr key={row.id} className="hover:bg-gray-800/30 transition-colors">
                  ...
                  <td className="py-3 px-4 text-right font-mono font-bold text-primary">{Number(row.balance).toLocaleString()} {row.balance > 0 ? 'Dr' : 'Cr'}</td>
                </tr>
              ))}
              {ledger.length === 0 && (
                <tr>
