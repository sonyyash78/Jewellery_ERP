...
import { Package, PlusCircle, Search, Filter, Printer, Trash2, Download } from 'lucide-react';
...
const InventoryTableRow = ({ item, onDelete, onDownload }: { item: StockItem, onDelete: (id: number) => void, onDownload: (path: string, code: string) => void }) => {
  ...
  return (
    <tr className="hover:bg-gray-800/30 transition-colors">
      ...
      <td className="py-3 px-4 font-mono text-primary font-bold">{item.net_weight}g</td>
      <td className="py-3 px-4">
      ...
      <div className="flex-1 bg-surface border border-gray-800 rounded-xl shadow-lg flex flex-col overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-800 flex gap-4">
          ...
          <div className="relative">
            <Filter className="absolute left-3 top-2 text-gray-500" size={18} />
            <select 
              value={metalFilter} onChange={(e) => setMetalFilter(e.target.value)}
              className="bg-background border border-gray-700 rounded-lg pl-10 pr-8 py-2 text-sm text-textMain focus:border-primary outline-none appearance-none cursor-pointer"
            >
